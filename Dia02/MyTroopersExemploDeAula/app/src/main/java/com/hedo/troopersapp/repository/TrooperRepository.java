package com.hedo.troopersapp.repository;
...
import com.hedo.troopersapp.util.SharedPreferencesUtil;
...
public class TrooperRepository {
    ...
    public static ArrayList<Trooper>
        tryGettingFromSharedPreferences(SharedPreferences sharedPreferences) {

        Gson gson = new Gson();

        SharedPreferencesUtil util =
                new SharedPreferencesUtil(sharedPreferences);

        if (util.hasValue(Constants.TROOPER_LIST)) {
            String json = util.get(Constants.TROOPER_LIST);
            return gson.fromJson(json,
                            new TypeToken<ArrayList<Trooper>>() {
                    }.getType());
        }
